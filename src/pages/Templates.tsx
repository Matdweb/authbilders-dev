            ...
            {templates?.map((template, index) => (
              <Card key={index} className={`flex flex-col h-full ${template.isInProgress && 'opacity-50 pointer-events-none'}`}>

                {
                  ...
                  (
                    <div className="relative bottom-1 right-1 max-w-32 text-center text-xs bg-orange-400 text-white px-2 py-0.5 rounded">
                      Coming Soon 🚧
                    </div>
                  )
                }
                <CardHeader>
                  <div className="flex justify-between items-start">
                    ...
                    <span className={`px-2 py-1 text-xs font-medium rounded-full ${template.difficulty === "Beginner"
                ...
                <CardFooter className="flex gap-2 pt-2">
                  <Button variant="outline" className="flex-1" asChild>
                    <Link to={template.docUrl}>Docs</Link>
                  </Button>
